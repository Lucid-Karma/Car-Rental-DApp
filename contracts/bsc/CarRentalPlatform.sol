...
contract CarRentalPlatform is ReentrancyGuard {
  ...
  //editCarMetada #onlyOwner #existingCar
  function editCarMetadata(uint id, string calldata name, string calldata imgUrl, uint rentFee, uint saleFee) external onlyOwner {
    require(cars[id].id != 0, "Car with given ID is does not exist.");
  ...
  //checkOut #existingUser #isCarAvailable #userHasNotRentedCar #userhasNoDebt
  ...
  function getContractBalance() external view onlyOwner returns(uint) {       //the whole contract balance with users balance
    return address(this).balance;
  }

  //getTotalPayment #onlyOwner
  function getTotalPayment() external view onlyOwner returns(uint) {          //the total debt payments.
