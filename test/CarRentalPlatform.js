const CarRentalPlatform = artifacts.require("CarRentalPlatform"); //./CarRentalPlatform.sol
...
contract("CarRentalPlatform", accounts => {
  ...
  describe("Deposit token and make payement", () => {
    ...
    it("checks in a car", async () => {
      ...
      assert.equal(user.debt, 0, "Something went wrong while trying to make the payement");
    });
  });
  ...
  describe("edit car", () => {
    it("should edit an existing car's metadata with valid parameters", async () => {
      ...
      assert.equal(car.saleFee, newSaleFee, "Problem editing car sale fee");
    });
  ...
  describe("Withdraw balance", () => {
    it("should send the desired amounts of tokens to the user", async () => {
      await carRentalPlatform.addUser("Alice", "Smith", {from: user1} );
      await carRentalPlatform.deposit({ from: user1, value: 100});
      await carRentalPlatform.withdrawBalance(50, {from: user1});

      const user = await carRentalPlatform.getUser(user1);
      assert.equal(user.balance, 50, "User could not get her/his tokens");
    });

    it("should send the desired amounts of tokens to the owner", async () => {
      await carRentalPlatform.addUser("Alice", "Smith", {from: user1} );
